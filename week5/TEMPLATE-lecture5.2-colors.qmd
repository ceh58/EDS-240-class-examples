...
cat_color_plot +
  scale_color_viridis_d() # discrete data/categorical

# continuous color scales ----
con_color_plot +
  scale_color_viridis_c(option = "magma", direction = -1) # continuous data, specify scale "magma", flip direction
```
...
RColorBrewer::display.brewer.pal(n = 4, name = "Dark2") # n = 4, name = "Dark2" (4 colors, from dark2 palette)
...
cat_color_plot +
  scale_color_brewer(palette = "Dark2") # or fill

# for unclassed continuous color scales ----
con_color_plot +
  scale_color_distiller(palette = "BuPu") # continuous

# for classed continuous color scales ----
con_color_plot +
  scale_color_fermenter(palette = "YlGnBu") # binned
```
...
# create palette ----
pal_discrete <- paletteer_d("wesanderson::GrandBudapest1", n = 3)

# apply to scatter plot (use `color` variant) ----
cat_color_plot +
  scale_color_manual(values = pal_discrete) # categorical
...
# create palette ----
pal_continuous <- paletteer_c("scico::grayC", n = 5, direction = -1)

# apply to scatter plot as an unclassed palette (use `gradientn` variant) ---- 
con_color_plot +
  scale_color_gradientn(colors = pal_continuous)

# apply to scatter plot as a classed (binned) palette (use `stepsn` variant) ---- 
con_color_plot +
  scale_color_stepsn(colors = pal_continuous) # binned color scale
```
