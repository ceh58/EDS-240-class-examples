...
ggplot() +
  ...
  geom_point(data = penguins_summary, 
             mapping = aes(x = mean_bill_length_mm, y = mean_bill_depth_mm, 
                           fill = species),
             size = 6,
             shape = 24)
```
...
#| warning: false
ggplot(penguins, aes(x = species, fill = island)) +
  geom_bar(position = position_dodge2(preserve = "single")) # Makes bars the same width
```
