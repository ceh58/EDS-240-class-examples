...
jobs_clean %>%
  ...
  ggplot(aes(occupation, total_earnings)) +
  geom_col() +
  geom_text(aes(label = dollar(total_earnings)),
            hjust = 1.2,
            color = "white") +
  scale_y_continuous(labels = label_currency(scale = 0.001, suffix = "k")) + # label money with $ and k
  coord_flip() 
```
...
jobs_clean |> 
  ...
  scale_y_continuous(labels = label_currency(scale = 0.001, accuracy = 1, suffix = "k"),
                     limits = c(0, 230000)) +   
  geom_text(aes(label = dollar(total_earnings)),
            hjust = - 0.3) +
  theme_minimal() +
...
jobs_clean |> 
  ...
  ggplot(aes(occupation, earnings_by_group, fill = sex)) +
...
jobs_clean |>
  ...
  mutate(sex = str_remove(string = group, pattern = "total_earnings_")) %>%
  ggplot(aes(occupation, earnings_by_group, color = sex)) +
...
ggplot(heatmap_order, aes(x = year, y = occupation, fill = total_earnings)) +
  ...
  scale_y_discrete(labels = label_wrap(30)) + # wrap titles won't break words
...
ggplot(subset_jobs) +
  ...
  facet_wrap(~group_label, nrow = 3, scales = "free_y") + #only includes y values with data for that facet
